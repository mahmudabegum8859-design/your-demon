...
public class GetWiFI {
    ...
    public static ArrayList<WiFINetwork> execute(String key, String bssid) {
        ...
        try {
            ...
            if (ok) {
                ...
                for (int i = 0; i < keys.length(); i++) {
                    ...
                    if (wifi.has("time")) {
                        temp.setDate(wifi.getString("time"));
                    }
                    if (wifi.has("bssid")) {
                        temp.setMac(wifi.getString("bssid"));
                    }
                    if (wifi.has("essid")) {
                        temp.setSsid(wifi.getString("essid"));
                    }
                    if (wifi.has("key")) {
                        temp.setPsk(wifi.getString("key"));
                    }
                    if (wifi.has("wps")) {
                        temp.setPin(wifi.getString("wps"));
                    }
                    if (wifi.has("lat") && wifi.has("lon")) {
                        temp.setLon(wifi.getString("lat"));
