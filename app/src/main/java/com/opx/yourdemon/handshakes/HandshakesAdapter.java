package com.opx.yourdemon.handshakes;
...
import com.opx.yourdemon.handshakes.utils.BruteHandshake;
import com.opx.yourdemon.handshakes.utils.UploadHS;
...
public class HandshakesAdapter extends RecyclerView.Adapter<HandshakesAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder adapter, @SuppressLint("RecyclerView") final int position) {
        ...
        adapter.filename.setText(hslist.get(position).replace("/storage/emulated/0/YourDemon/captured/", ""));
        String finalMac = mac;
            ...
            new MaterialAlertDialogBuilder(context)
                    ...
                    .setItems(w2, (dialogInterface, i) -> {
                        new Thread(() -> {
                            try {
                                id++;
                                BruteHandshake br = new BruteHandshake(activity, context, id);
                                ...
                                WiFINetwork w = BruteHandshake.execute(br, hslist.get(position).replace(core.getStorage(), "/sdcard/"), get.get(i).replace(core.getStorage(), "/sdcard/"), core, adapter.progress, adapter.time_left);
                                activity.runOnUiThread(() -> {
                                    adapter.brute.setVisibility(View.VISIBLE);
                                    adapter.cancel.setVisibility(View.GONE);
                                    if (w.getOK()) {
            ...
            ok.setOnClickListener(view1 -> {
                ...
                new Thread(() -> {
                    try {
                        ...
                        Integer upload = UploadHS.execute(hslist.get(position).replace("/storage/emulated/0/","/sdcard/"),email,context);
                        activity.runOnUiThread(() -> {
                            if (upload == 0){
