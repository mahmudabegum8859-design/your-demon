...
public class SlideUpdate extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        button.setOnClickListener(view1 -> {
            ...
            try {
                boolean kill = CustomCommand.execute("/data/data/com.opx.yourdemon/files/killrootold", new Core(context));
                if (!CheckDir.check("/data/local/YourDemon/beta/sdcard/YourDemon")){
                    boolean rm = CustomCommand.execute("rm -rf /data/local/YourDemon/beta", new Core(context));
                    ...
                    button.setOnClickListener(view2 -> {
                        try {
                            boolean reboot = CustomCommand.execute("reboot", new Core(context));
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    });
